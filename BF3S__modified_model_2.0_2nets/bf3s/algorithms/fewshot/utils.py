...
def few_shot_feature_classification(
    classifier, features_test_1, features_train_1, features_test_2 , features_train_2 ,labels_train_1hot, labels_test,  base_ids=None
    
):
    """Applies the classification head of few-shot classification model."""
    # print("feature_train", features_train.size())
    # print("labels_test", labels_test.size())
    # print("features_test",features_test.size())
    if base_ids is not None:
        classification_scores = classifier(
            features_test_1=features_test_1,
            features_train_1=features_train_1,
            features_test_2=features_test_2,
            features_train_2=features_train_2,
    ...
    # print("classification score dim",classification_scores.dim())

    # print("classification score",classification_scores.size())
    labels_test = labels_test.view(-1)
    # print("labels test", labels_test.size())
    assert classification_scores.size(0) == labels_test.size(0)
    # print("tset", classification_scores)
    # print("labels test test", labels_test)
...
def compute_accuracy_metrics(scores, labels, num_base, record={}, string_id=""):
    ...
    record["AccuracyBase" + string_id] = utils.top1accuracy(scores, labels)
    ...
    #     # print("novel_indcies", novel_indices)
...
def fewshot_classification(
    feature_extractor_1,
    feature_extractor_optimizer_1,
    feature_extractor_2,
    feature_extractor_optimizer_2,
    classifier,
    ...
    if is_train:  # zero the gradients
        if feature_extractor_optimizer_1:
            feature_extractor_optimizer_1.zero_grad()
        if feature_extractor_optimizer_2:
            feature_extractor_optimizer_2.zero_grad()
    ...
    train_feature_extractor_1 = is_train and (feature_extractor_optimizer_1 is not None)
    train_feature_extractor_2 = is_train and (feature_extractor_optimizer_2 is not None)
    with torch.set_grad_enabled(train_feature_extractor_1):
        # Extract features from the train and test images.
        features_1 = cls_utils.extract_features(
            feature_extractor_1, images, feature_name=feature_name
        )
    with torch.set_grad_enabled(train_feature_extractor_2):
        # Extract features from the train and test images.
        features_2 = cls_utils.extract_features(
            feature_extractor_2, images, feature_name=feature_name
        )


    if not train_feature_extractor_1:
        # Make sure that no gradients are backproagated to the feature
        # extractor when the feature extraction model is freezed.
        features_1 = features_1.detach()

    if not train_feature_extractor_2:
        features_2 = features_2.detach()
    # print("features1", features_1.size())
    # print("features2", features_2.size())
    # features = features_1
    # features = torch.cat([features_1, features_2], dim=1)
    # print("concate", features.size())

    with torch.set_grad_enabled(is_train):
        features_train_1 = features_1[:batch_size_train]
        features_test_1 = features_1[batch_size_train:]
        features_train_1 = utils.add_dimension(features_train_1, meta_batch_size)
        features_test_1 = utils.add_dimension(features_test_1, meta_batch_size)
        features_train_2 = features_2[:batch_size_train]
        features_test_2 = features_2[batch_size_train:]
        features_train_2 = utils.add_dimension(features_train_2, meta_batch_size)
        features_test_2 = utils.add_dimension(features_test_2, meta_batch_size)
    ...
    if is_train:
        loss_total.backward()
        if feature_extractor_optimizer_1:
            feature_extractor_optimizer_1.step()
        if feature_extractor_optimizer_2:
            feature_extractor_optimizer_2.step()
        classifier_optimizer.step()
    record['AccuracyNovel'] = record['AccuracyBase']
    return record
