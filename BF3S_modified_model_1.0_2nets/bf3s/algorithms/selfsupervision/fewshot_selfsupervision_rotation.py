...
class FewShotRotationSelfSupervision(algorithm.Algorithm):
    ...
    def set_tensors(self, batch):
        ...
        if two_datasets:
            ...
            # print(" labels_train_unsqueeze", labels_train_unsqueeze)
            # print("self num base", self.num_base)
            # print("test result", labels_train_unsqueeze - self.num_base)
            # print("labels_train_1hot_size", labels_train_1hot_size)
            # print("scatter test", len(labels_train_1hot_size) - 1)
    ...
    def  process_batch_fewshot_classification_task(self, auxiliary_rotation_task, is_train):
        ...
        if auxiliary_rotation_task:
            ...
            if self.semi_supervised:
                raise ValueError("Not supported option.")
            record, features_train_rot, features_test_rot = rot_utils.fewshot_classification_with_rotation_selfsupervision_2(
                feature_extractor=self.networks["feature_extractor_2"],
                feature_extractor_optimizer=self.optimizers.get("feature_extractor_2"),
                # classifier=self.networks["classifier"],
            ...
            record_2 = rot_utils.fewshot_classification_with_rotation_selfsupervision_1(
                features_train_rot = features_train_rot,
                features_test_rot = features_test_rot,
                feature_extractor=self.networks["feature_extractor_1"],
                feature_extractor_optimizer=self.optimizers.get("feature_extractor_1"),
                classifier=self.networks["classifier"],
                classifier_optimizer=self.optimizers.get("classifier"),
                # classifier_rot=self.networks["classifier_aux"],
                # classifier_rot_optimizer=self.optimizers.get("classifier_aux"),
            ...
            record = fs_utils.fewshot_classification(
                ...
                feature_extractor_1=self.networks["feature_extractor_1"],
                feature_extractor_optimizer_1=self.optimizers.get("feature_extractor_1"),
                feature_extractor_2=self.networks["feature_extractor_2"],
                feature_extractor_optimizer_2=self.optimizers.get("feature_extractor_2"),
                classifier=self.networks["classifier"],
                classifier_optimizer=self.optimizers.get("classifier"),
                images_train=self.tensors["images_train"],
                labels_train=self.tensors["labels_train"],
