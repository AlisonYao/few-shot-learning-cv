...
sys.path.append("../few-shot-learning-cv")
sys.path.extend([os.path.join(root, name) for root, dirs, _ in os.walk("../few-shot-learning-cv") for name in dirs])
...
print(str(DATA_PATH) + '/miniImageNet/images_background')
...
for c in background_classes:
    ...
    mkdir(DATA_PATH + '/miniImageNet/images_background/'+str(c)+'/')

for c in evaluation_classes:
    # mkdir(DATA_PATH + f'/miniImageNet/images_evaluation/{c}/')
    mkdir(DATA_PATH + '/miniImageNet/images_evaluation/'+str(c)+'/')
...
for root, _, files in os.walk(DATA_PATH + '/miniImageNet/images'):
    for f in tqdm(files, total=600*100):
        if f.endswith('.jpg'):
            ...
            dst = DATA_PATH + '/miniImageNet/'+str(subset_folder)+'/'+str(class_name)+'/'+str(image_name)
            shutil.copy(src, dst)
