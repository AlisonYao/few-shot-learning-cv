...
import bf3s.architectures as architectures
...
class Algorithm:
    ...
    def set_log_file_handler(self, _log=None):
        ...
        self.tensorboard_writer = tensorboardX.SummaryWriter(str(tensorboard_writer_path))
        self.logger.info(f"==> Tensorboard file location: {tensorboard_writer_path}")
    ...
    def load_network(self, net_key, epoch, suffix=""):
        assert net_key in self.networks
        filename = pathlib.Path(self._get_net_checkpoint_filename(net_key, epoch) + suffix)
    ...
    def load_optimizer(self, net_key, epoch, suffix=""):
        assert net_key in self.optimizers
        filename = pathlib.Path(self._get_optim_checkpoint_filename(net_key, epoch) + suffix)
        assert filename.is_file()
        checkpoint = torch.load(filename)
    ...
    def add_stats_to_tensorboard_writer(self, stats, prefix):
        for key, value in stats.items():
            if isinstance(value, dict):
                self.add_stats_to_tensorboard_writer(value, prefix + key + "_")
            else:
                self.tensorboard_writer.add_scalar(prefix + key, value, self.curr_epoch)

    def adjust_learning_rates(self, epoch):
        # filter out the networks that are not trainable and that do
        # not have a learning rate Look Up Table (LUT_lr) in their optim_params
        optim_params_filtered = {
            k: v for k, v in self.optim_params.items() if (v is not None and ("LUT_lr" in v))
        }
